...
const getUserProfile = async (req, res) => {
	...
	try {
		...
		// Send the user profile, roles are included here
		res.status(200).json(user);
	} catch (err) {
		res.status(500).json({ error: err.message });
		console.log("Error in getUserProfile: ", err.message);
...
// Additional controller actions for login, logout, follow/unfollow, etc.
// Including freeze account, award verification, and fetching suggested users
...
export { getUserProfile, signupUser, followUnFollowUser, freezeAccount, awardVerification };
