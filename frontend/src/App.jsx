...
import i18n from './i18n';
import ReviewModal from './components/ReviewModal';
import MagicLogin from './components/MagicLogin'; // Changed from pages to components
      ...
      <Box>
        ...
        <Box mx="auto" px={4} maxW={isTVPage ? "100vw" : "600px"}>
          <Routes>
            <Route path="/" element={user ? <HomePage /> : <Navigate to="/auth" />} />
            <Route path="/auth" element={!user ? <AuthPage /> : <Navigate to="/" />} />
            <Route path="/auth/magic-login" element={<MagicLogin />} /> {/* Keep as a route, but now it's a component */}
            <Route path="/update" element={user ? <UpdateProfilePage /> : <Navigate to="/auth" />} />
