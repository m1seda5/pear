...
// admin role update
import { Button, Flex, Image, Link, useColorMode, keyframes } from "@chakra-ui/react";
import { useRecoilValue, useSetRecoilState } from "recoil";
import userAtom from "../atoms/userAtom";
import { AiFillHome } from "react-icons/ai";
import { RxAvatar } from "react-icons/rx";
...
// Icy blue animation
const icyBlueAnimation = keyframes`
  0% { color: #87CEEB; }
  50% { color: #ADD8E6; }
  100% { color: #87CEEB; }
`;
...
const Header = () => {
  ...
  const handleChatClick = (e) => {
    if (!hasChatAccess || user.isFrozen) {
      e.preventDefault(); // Prevent navigation if the user doesn't have access
      setHoveringLock(true); // Show lock when hovering
    } else {
      setHoveringLock(false);
      navigate("/chat"); // Navigate to chat page if access is allowed
    }
      ...
      {user && (
        <Flex alignItems="center" gap={10}>
          ...
          <Link
            onClick={handleChatClick}
            _hover={{
              color: hasChatAccess && !user.isFrozen ? "teal.500" : user.isFrozen ? "#87CEEB" : "red.500",
              transform: "scale(1.2)",
              cursor: hasChatAccess && !user.isFrozen ? "pointer" : "not-allowed",
            }}
            transition="all 0.3s ease-in-out"
            onMouseEnter={() => (!hasChatAccess || user.isFrozen) && setHoveringLock(true)}
            onMouseLeave={() => setHoveringLock(false)}
          >
            {hoveringLock || user.isFrozen ? (
              <FaLock
                size={20}
                style={{
                  animation: user.isFrozen ? `${icyBlueAnimation} 2s infinite` : undefined,
                  color: user.isFrozen ? "#87CEEB" : "red",
                }}
              />
            ) : (
              <BsFillChatQuoteFill size={20} />
