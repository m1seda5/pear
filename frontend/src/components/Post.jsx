...
const Post = ({ post, postedBy }) => {
  ...
  useEffect(() => {
    const getUser = async () => {
      try {
        console.log("Posted by:", postedBy);  // Log postedBy to verify its content

        // Ensure you're using either the ID or username from postedBy
        const userId = postedBy.id || postedBy.username;
        if (!userId) {
